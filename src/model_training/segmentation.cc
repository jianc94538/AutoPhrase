...
void Segmentation::normalizePosTags() {
  for (int i = 0; i < connect.size(); ++i) {
    double sum = 0;
    for (int j = 0; j < connect[i].size(); ++j) {
      sum += connect[i][j];
...
double Segmentation::viterbi_proba(const vector<TOKEN_ID_TYPE> &tokens,
                                   vector<double> &f, vector<int> &pre) {
  f.clear();
...
double
Segmentation::viterbi_proba_randomPOS(const vector<TOKEN_ID_TYPE> &tokens,
                                      vector<double> &f, vector<int> &pre) {
  FrequentPatternMining::Pattern pattern;
  for (size_t i = 0; i < tokens.size(); ++i) {
  ...
  for (TOTAL_TOKENS_TYPE ed : pattern_mining_->id2ends[id]) {
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
      double cost = 1;
      for (size_t j = i, u = 0; j < tokens.size() && j - i + 1 < tokens.size();
           ++j) {
        if (!trie[u].children.count(tokens[j])) {
          assert(j != i);
          break;
...
void Segmentation::rectifyFrequency(
