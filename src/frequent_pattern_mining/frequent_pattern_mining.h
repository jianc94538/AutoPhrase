...
public:
  ...
  inline void addPatternWithoutLocks(const Pattern &pattern,
  ...
  inline void addPattern(const Pattern &pattern, const TOTAL_TOKENS_TYPE &ed,
                         bool addPosition = true) {
    assert(pattern2id.count(pattern.hashValue));
    PATTERN_ID_TYPE id = pattern2id[pattern.hashValue];
    assert(id < id2ends.size());
  ...
  inline void mine(const Configure& config) {
...
# pragma omp parallel for schedule(dynamic, PATTERN_CHUNK_SIZE) reduction(+:totalOcc)
    ...
    for (int len = 1; len <= LENGTH_THRESHOLD && last < patterns.size();
         ++len) {
      if (INTERMEDIATE) {
