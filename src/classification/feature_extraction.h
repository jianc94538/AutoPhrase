...
public:
  ...
  void extractPunctuation(PATTERN_ID_TYPE id, vector<double> &feature) {
    ...
    for (const TOTAL_TOKENS_TYPE &ed : id2ends[id]) {
      ...
      assert(corpus_->wordTokens[st] ==
             pattern_mining_->patterns[id].tokens[0]);
  ...
  void extractStatistical(PATTERN_ID_TYPE id, vector<double> &feature) {
    ...
    // prob_feature
    double f1 = pattern.probability / patterns[AB].probability /
                patterns[CD].probability;
    // occur_feature
    double f2 = patterns[id].currentFreq /
                sqrt(patterns[AB].currentFreq + EPS) /
...
#pragma omp parallel for schedule(dynamic, PATTERN_CHUNK_SIZE)
  ...
  vector<vector<double>> extractUnigram(vector<string> &featureNames) {
    ...
    const TOTAL_TOKENS_TYPE &corpusTokensN = corpus_->totalWordTokens;
    for (PATTERN_ID_TYPE i = 0; i < patterns.size(); ++i) {
      patterns[i].probability = patterns[i].currentFreq /
                                (corpusTokensN / (double)patterns[i].size());
    }
