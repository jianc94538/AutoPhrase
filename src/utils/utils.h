...
inline string replaceAll(const string &s, const string &from,
                         const string &to) {
  string ret = "";
...
template <class T> inline void fromString(const string &s, T &x) {
  stringstream in(s);
  in >> x;
}
...
namespace Binary {
template <class T> inline void write(FILE *out, const T &x) {
  fwrite(&x, sizeof(x), 1, out);
}

template <class T> inline void read(FILE *in, T &size) {
